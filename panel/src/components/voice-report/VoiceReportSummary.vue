...
const kpiCards = computed(() => [
  {
    ...
    value: latencyAvg.value,
    unit: 'ms',
    sub: `min ${metrics.value.latency?.min ?? 0} / max ${metrics.value.latency?.max ?? 0} ms`,
    st: statusFor(latencyAvg.value, thresholds.value.warning_jitter_ms * 5, thresholds.value.warning_jitter_ms * 8),
  },
  {
    label: 'Avg Jitter',
    value: metrics.value.jitter?.avg ?? 0,
    unit: 'ms',
    sub: `peak ${jitterMax.value} ms`,
  ...
  },
  {
    ...
    value: lossPct,
    unit: '%',
    sub: `${metrics.value.packets?.lost ?? 0} of ${(metrics.value.packets?.sent ?? 0).toLocaleString()} packets`,
    st: statusFor(lossPct, thresholds.value.warning_loss_pct * 0.5, thresholds.value.warning_loss_pct),
  },
  {
    ...
    st: statusFor(dupPct, thresholds.value.critical_loss_pct, thresholds.value.critical_loss_pct * 5),
  },
  {
    label: 'Out-of-Order',
    value: oooPct,
    ...
    st: statusFor(oooPct, thresholds.value.out_of_sequence_pct, thresholds.value.out_of_sequence_pct * 4),
  },
  {
